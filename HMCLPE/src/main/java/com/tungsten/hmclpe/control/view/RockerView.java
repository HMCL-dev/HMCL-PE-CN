...
public class RockerView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mOutPath.moveTo(mAreaRadius, mAreaRadius >> 2);
        mOutPath.lineTo(mAreaRadius - (mRockerRadius >> 1), (mAreaRadius >> 2) + (mRockerRadius >> 2));
        mOutPath.moveTo(mAreaRadius, mAreaRadius >> 2);
        mOutPath.lineTo(mAreaRadius + (mRockerRadius >> 1), (mAreaRadius >> 2) + (mRockerRadius >> 2));
        mOutPath.moveTo(mAreaRadius >> 2,mAreaRadius);
        mOutPath.lineTo((mAreaRadius >> 2) + (mRockerRadius >> 2),mAreaRadius - (mRockerRadius >> 1));
        mOutPath.moveTo(mAreaRadius >> 2,mAreaRadius);
        mOutPath.lineTo((mAreaRadius >> 2) + (mRockerRadius >> 2),mAreaRadius + (mRockerRadius >> 1));
        mOutPath.moveTo(mAreaRadius, 7 * mAreaRadius >> 2);
        mOutPath.lineTo(mAreaRadius - (mRockerRadius >> 1), (7 * mAreaRadius >> 2) - (mRockerRadius >> 2));
        mOutPath.moveTo(mAreaRadius, 7 * mAreaRadius >> 2);
        mOutPath.lineTo(mAreaRadius + (mRockerRadius >> 1), (7 * mAreaRadius >> 2) - (mRockerRadius >> 2));
        mOutPath.moveTo(7 * mAreaRadius >> 2,mAreaRadius);
        mOutPath.lineTo((7 * mAreaRadius >> 2) - (mRockerRadius >> 2),mAreaRadius - (mRockerRadius >> 1));
        mOutPath.moveTo(7 * mAreaRadius >> 2,mAreaRadius);
        mOutPath.lineTo((7 * mAreaRadius >> 2) - (mRockerRadius >> 2),mAreaRadius + (mRockerRadius >> 1));

        mInnerPath.moveTo(mAreaRadius,mAreaRadius - 2 * mRockerRadius / 3);
        mInnerPath.lineTo(mAreaRadius + 2 * mRockerRadius / 3,mAreaRadius);
        mInnerPath.lineTo(mAreaRadius,mAreaRadius + 2 * mRockerRadius / 3);
        mInnerPath.lineTo(mAreaRadius - 2 * mRockerRadius / 3,mAreaRadius);
        mInnerPath.lineTo(mAreaRadius,mAreaRadius - 2 * mRockerRadius / 3);

        mDirectionPaint.setStyle(Paint.Style.STROKE);
        canvas.drawPath(mOutPath,mDirectionPaint);
        canvas.drawPath(mInnerPath,mDirectionPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:// 按下
                initialPoint.set((int) this.getX(),(int) this.getY());
                downX = event.getX();
                downY = event.getY();
                downTime = System.currentTimeMillis();
                isClick = true;
                float lenX = (float) (downX - mCenterPoint.x);
                float lenY = (float) (downY - mCenterPoint.y);
                float lenXY = (float) Math.sqrt((double) (lenX * lenX + lenY * lenY));
                if (lenXY <= followRadius){
                    this.setX(initialPoint.x + lenX);
                    this.setY(initialPoint.y + lenY);
                }
                callBackStart();
            ...
            case MotionEvent.ACTION_CANCEL:// 移出区域
                // 回调 结束
                callBackFinish();
                this.setX(initialPoint.x);
                this.setY(initialPoint.y);
                upX = event.getX();
                upY = event.getY();
                ...
                if (isClick){
                    mOnShakeListener.onRockerClick(this);
                }
...
/     * 获取摇杆实际要显示的位置（点）
    ...
    private void callBackStart() {
        ...
        if (null != mOnShakeListener) {
            mOnShakeListener.onRockerStart(this);
        }
    }
     ...
     * @param angle 摇动角度
     */
    private void callBack(double angle,float lenXY) {
        if (null != mOnShakeListener && lenXY >= mAreaRadius / 3) {
            isClick = false;
            if ((ANGLE_0 <= angle && ANGLE_8D_OF_0P > angle || ANGLE_8D_OF_7P <= angle && ANGLE_360 > angle) && tempDirection != Direction.DIRECTION_RIGHT) {
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_RIGHT);
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_DOWN_RIGHT);
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_DOWN);
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_DOWN_LEFT);
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_LEFT);
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_UP_LEFT);
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_UP);
                ...
                mOnShakeListener.onRockerShake(this, Direction.DIRECTION_UP_RIGHT);
            }
        }
        else if (null != mOnShakeListener && lenXY < mAreaRadius / 3){
            tempDirection = Direction.DIRECTION_CENTER;
            mOnShakeListener.onRockerShake(this, Direction.DIRECTION_CENTER);
        }
    }
    ...
    private void callBackFinish() {
        tempDirection = Direction.DIRECTION_CENTER;
        if (null != mOnShakeListener) {
            mOnShakeListener.onRockerFinish(this);
        }
    }
     ...
     * @param listener      回调
     */
    public void setOnShakeListener(OnShakeListener listener,int followRadius) {
    ...
    public interface OnShakeListener {
        ...
        void onRockerClick(RockerView rockerView);

        void onRockerStart(RockerView rockerView);

        void onRockerShake(RockerView rockerView,Direction direction);

        void onRockerFinish(RockerView rockerView);
    }
