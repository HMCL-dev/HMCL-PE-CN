 ...
 */
package com.tungsten.hmclpe.multiplayer.hiper;
...
public class LocalServerBroadcaster implements AutoCloseable {
    ...
    private void broadcastMOTD(int port) {
        ...
        while (running) {
            try {
                byte[] data = String.format("[MOTD]%s[/MOTD][AD]%d[/AD]", "HMCL-PE Multiplayer Session", port).getBytes(StandardCharsets.UTF_8);
